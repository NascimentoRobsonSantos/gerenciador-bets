...
export default function Header() {
  ...
  const handleThemeChange = async (newTheme: string) => {
    ...
    try {
      await fetch('/settings', {
        ...
        body: JSON.stringify({ dark_theme: newTheme === 'dark' }),
      });
    } catch (error) {
  ...
  return (
    <header className="bg-card text-card-foreground p-4 flex justify-between items-center shadow-md">
      <h1 className="text-xl font-bold">Gerenciador de Pedidos</h1>
      <nav>
        <ul className="flex space-x-4">
          <li><a href="#" className="hover:text-primary">Início</a></li>
          <li><a href="#" className="hover:text-primary">Pedidos</a></li>
